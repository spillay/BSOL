/*
 ...
 Version     :
 Copyright   : DSLEng
 Description : Uses shared library libsptun to create a server implementation
 ============================================================================
 */
...
int main(int argc, char *argv[], char *env[]) {
	vtun_syslog(LOG_INFO, "Starting SPServer");
	...
	init();
	// SP: In this code SVR is always 1
	svr = 1;
	...
	init_title(argc, argv, env, "SPServer: ");
	write_pid();
	server(sock);
	closelog();
	clean();
...
void usage(void) {
	...
	printf("Usage: \n");
	printf("  SPServer:\n");
	printf("\tSPServer [-f file] [-P port] [-L local address]\n");
}
