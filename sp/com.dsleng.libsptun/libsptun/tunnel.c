...
int tunnel(struct vtun_host *host) {
	...
	if ((host->persist == VTUN_PERSIST_KEEPIF) && (host->loc_fd >= 0))
	...
	if (!interface_already_open) {
		...
		case VTUN_TTY:
			if ((fd[0] = pty_open(dev)) < 0) {
				vtun_syslog(LOG_ERR, "Can't allocate pseudo tty. %s(%d)",
		...
		case VTUN_PIPE:
			if (pipe_open(fd) < 0) {
				vtun_syslog(LOG_ERR, "Can't create pipe. %s(%d)",
						strerror(errno), errno);
				return -1;
			}
			break;
		...
		case VTUN_ETHER:
			if ((fd[0] = tap_open(dev)) < 0) {
				vtun_syslog(LOG_ERR, "Can't allocate tap device %s. %s(%d)",
		...
		case VTUN_TUN:
			if ((fd[0] = tun_open(dev)) < 0) {
				vtun_syslog(LOG_ERR, "Can't allocate tun device %s. %s(%d)",
						dev, strerror(errno), errno);
				return -1;
			}
			break;
	...
	case 0:
		...
		vtun_syslog(LOG_INFO, "Before processing commands");
