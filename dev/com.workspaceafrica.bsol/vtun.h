...
#include "llist.h"
#include "bsol/Client/fd_manager.h"
...
struct vtun_host {
	...
	int ctrl;
	int nl;
	struct sockaddr_in rmt_sock;
	struct sockaddr_in rmt_sock2;
	int ka_failure;

	struct vtun_addr saddr[VTUN_MAX_INT]; /* tunnel address */
	int sport[VTUN_MAX_INT]; /* tunnel port */
	struct vtun_addr daddr[VTUN_MAX_INT]; /* tunnel address */
	int dport[VTUN_MAX_INT]; /* tunnel port */

	//add by Kai for multiple interfaces
	int fscount;
	fd_state fs[VTUN_MAX_INT];
	int ifa_index_hash[VTUN_MAX_SYS_INT];
	int rmt_int;
